"""cater-waiter
exercises on sets under the guise of a catering business
"""
...
def check_drinks(drink_name, drink_ingredients):
    ...
    alcoholic_ingredients = set(drink_ingredients).intersection(ALCOHOLS)
    if len(alcoholic_ingredients) == 0:
...
def tag_special_ingredients(dish):
    ...
    allergens = set(dish[1])
    allergens = allergens.intersection(SPECIAL_INGREDIENTS)
...
def separate_appetizers(dishes, appetizers):
    ...
    dishes_unique = set(dishes)
    appetizers_unique = set(appetizers)
    return list(dishes_unique.difference(appetizers_unique))
...
def singleton_ingredients(dishes, intersection):
    ...
    ret = set()
    for dish in dishes:
        ret = ret.symmetric_difference(dish)
    return ret.difference(intersection)
